...
data class CustomData(val aaa: String)

@Suppress("UNUSED")
class CustomDataSerializer : SerializationCustomSerializer<CustomData, CustomDataSerializer.Proxy> {
    data class Proxy(val aaa: String) {
    }

    @Suspendable
    override fun fromProxy(proxy: Proxy) = CustomData(proxy.aaa)

    @Suspendable
    override fun toProxy(obj: CustomData) = Proxy(obj.aaa)
}
